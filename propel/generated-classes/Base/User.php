...
abstract class User implements ActiveRecordInterface
{
    ...
    /**
     * @var        ObjectCollection|ChildProfile[] Collection to store aggregation of ChildProfile objects.
     */
    ...
    public function setUserRole($v)
    {
        ...
        if ($this->role !== $v) {
            $this->role = $v;
            $this->modifiedColumns[] = UserTableMap::ROLE;
        }
    ...
    public function setIsVisible($v)
    {
        ...
        if ($this->visible !== $v) {
            $this->visible = $v;
            $this->modifiedColumns[] = UserTableMap::VISIBLE;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : UserTableMap::translateFieldName('UserRole', TableMap::TYPE_PHPNAME, $indexType)];
            $this->role = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : UserTableMap::translateFieldName('IsVisible', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::ROLE)) {
            $modifiedColumns[':p' . $index++]  = 'ROLE';
        }
        if ($this->isColumnModified(UserTableMap::VISIBLE)) {
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[5], $arr)) $this->setUserRole($arr[$keys[5]]);
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::ROLE)) $criteria->add(UserTableMap::ROLE, $this->role);
        if ($this->isColumnModified(UserTableMap::VISIBLE)) $criteria->add(UserTableMap::VISIBLE, $this->visible);
    ...
    /**
     * Gets an array of ChildProfile objects which contain a foreign key that references this object.
     *
     ...
     * @return Collection|ChildProfile[] List of ChildProfile objects
     * @throws PropelException
     */
        ...
        if (null === $this->collProfiles || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collProfiles) {
                ...
                if (null !== $criteria) {
                    ...
                    $collProfiles->getInternalIterator()->rewind();
    ...
    {
        $profilesToDelete = $this->getProfiles(new Criteria(), $con)->diff($profiles);


        $this->profilesScheduledForDeletion = $profilesToDelete;

        foreach ($profilesToDelete as $profileRemoved) {
            $profileRemoved->setUser(null);
        }
     ...
     * @throws PropelException
     */
    public function countProfiles(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collProfilesPartial && !$this->isNew();
        if (null === $this->collProfiles || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collProfiles) {
    ...
    /**
     * Method called to associate a ChildProfile object to this object
     * through the ChildProfile foreign key attribute.
     *
     * @param    ChildProfile $l ChildProfile
     * @return   \User The current object (for fluent API support)
     */
    public function addProfile(ChildProfile $l)
    {
        if ($this->collProfiles === null) {
            $this->initProfiles();
            $this->collProfilesPartial = true;
    ...
    public function removeProfile($profile)
    {
        if ($this->getProfiles()->contains($profile)) {
            ...
            $this->profilesScheduledForDeletion[]= clone $profile;
